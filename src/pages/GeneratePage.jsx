...
export default function GeneratePage() {
  const { models, defaults, comfyui, setCheckpoints, favoriteCheckpoints, toggleFavoriteCheckpoint } = useSettingsStore()
  ...
  // View tags state
  const [viewTags, setViewTags] = useState({ angle: [], camera: [], background: [], style: [] })
  ...
  const [progressDetails, setProgressDetails] = useState({ current: 0, max: 0 }) // Current step / Max steps
  ...
  // Sync checkpoints from ComfyUI API on mount
  useEffect(() => {
    const syncCheckpoints = async () => {
      try {
        const comfyURL = comfyui.apiUrl || 'http://127.0.0.1:8188'
        const response = await fetch(`${API_URL}/api/comfyui/checkpoints?comfyUIUrl=${encodeURIComponent(comfyURL)}`)

        if (response.ok) {
          const data = await response.json()
          if (data.checkpoints && data.checkpoints.length > 0) {
            // Transform checkpoint strings into model objects that SearchableModelDropdown expects
            const checkpointModels = data.checkpoints.map(ckptPath => {
              // Split path into folder and name (e.g., "Illustrious/reijlita/model.safetensors")
              const parts = ckptPath.split('/')
              const name = parts[parts.length - 1] // "model.safetensors"
              const folder = parts.length > 1 ? parts.slice(0, -1).join('/') : '' // "Illustrious/reijlita"

              return {
                type: 'checkpoint',
                name: ckptPath, // Full path for ComfyUI
                folder: folder,
                path: ckptPath,
                metadata: null
              }
            ...
            console.log('✅ Synced checkpoints from ComfyUI:', checkpointModels.length, 'models')
          }
        } else {
  ...
  const handleLoraSlotChange = (id, field, value) => {
    setLoraSlots(loraSlots.map(slot =>
  ...
  const connectToComfyUIWebSocket = (clientId) => {
    try {
      ...
      const wsUrl = comfyui.apiUrl.replace('http://', 'ws://').replace('https://', 'wss://') + '/ws'
        ...
        try {
          ...
          if (message.type === 'execution_error') {
            ...
            if (msgPromptId) {
              ...
              setGenerationError(message.data.exception_message || 'Generation failed')
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Build final prompt with view tags
      const viewTagsString = viewTagsArray.length > 0 ? viewTagsArray.join(', ') : ''
      const finalPrompt = viewTagsString
        ? `${prompt}, ${viewTagsString}`.trim()
        : prompt
  ...
  return (
    <div className="h-full">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        {/* Left Panel - Controls */}
        <div className="space-y-4 md:space-y-6 overflow-y-auto max-h-[calc(100vh-120px)]">
          {/* Header */}
          <div className="md:block hidden">
            ...
            <p className="text-sm md:text-base text-text-secondary">
              Configure your generation settings and create AI images
            </p>
          </div>

          {/* Prompts Section */}
          <div className="bg-bg-secondary rounded-lg border border-border-primary p-4 md:p-6 relative group">
            {/* Quick Generate Button (appears on hover) */}
            <motion.button
              ...
              className={`absolute top-4 right-4 w-10 h-10 rounded-full flex items-center justify-center transition-all z-10 ${
                isGenerating || !settings.checkpoint || !prompt.trim()
                  ...
                  : 'opacity-0 group-hover:opacity-100 bg-accent-success hover:bg-accent-success/90 text-white shadow-lg'
              }`}
              whileHover={{ scale: isGenerating || !settings.checkpoint || !prompt.trim() ? 1 : 1.1 }}
              whileTap={{ scale: isGenerating || !settings.checkpoint || !prompt.trim() ? 1 : 0.9 }}
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  {prompt && (
                    <motion.button
                      onClick={() => setPrompt('')}
                      className="px-2 py-1 text-xs text-text-tertiary hover:text-accent-error transition-colors"
              ...
              <TagAutocomplete
                ...
                placeholder="Describe what you want to generate... Type tags and autocomplete will suggest."
                rows={6}
              />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <WildcardMenu onInsert={(wildcard) => setNegativePrompt(negativePrompt + wildcard)} />
                </div>
              </div>
              <TagAutocomplete
          ...
          <div className="bg-bg-secondary rounded-lg border border-border-primary p-4 md:p-6">
            <div className="flex items-center justify-between mb-3 md:mb-4">
              <h2 className="text-lg md:text-xl font-semibold text-text-primary flex items-center gap-2">
                <span>🎛️</span>
                LoRA Slots
              </h2>
              <motion.button
                onClick={handleAddLoraSlot}
                className="px-3 md:px-4 py-2 md:py-2.5 bg-accent-primary text-white rounded-lg hover:bg-accent-primary/90 transition-colors text-sm font-medium touch-manipulation"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                + Add LoRA
            ...
            {loraSlots.length === 0 ? (
              ...
              <div className="space-y-3">
                {loraSlots.map((slot) => (
                  <div key={slot.id} className="bg-bg-tertiary rounded-lg p-4 border border-border-primary">
              ...
              <div className="col-span-2">
                ...
                <div className="flex gap-2 items-center mb-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 rounded-lg bg-bg-tertiary text-text-primary border border-border-primary focus:border-accent-primary focus:ring-2 focus:ring-accent-primary/20 transition-all duration-200 outline-none text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                  />
                  <motion.button
          ...
          <div className="bg-bg-secondary rounded-lg border border-border-primary p-4 md:p-6">
            ...
            <div className="mb-4">
              ...
              >
                ...
                <optgroup label="HD Widescreen (16:9)">
                  {resolutions.filter(r => r.category === 'HD Widescreen').map((res) => (
                    <option key={res.label} value={res.label}>{res.label}</option>
                  ))}
                </optgroup>
                <optgroup label="SD Legacy">
          ...
          <motion.button
            onClick={handleGenerate}
            disabled={isGenerating || !settings.checkpoint || !prompt.trim()}
            className={`w-full py-5 md:py-4 rounded-lg font-semibold text-lg transition-all duration-300 touch-manipulation ${
              isGenerating || !settings.checkpoint || !prompt.trim()
                ? 'bg-bg-tertiary text-text-tertiary cursor-not-allowed'
                : 'bg-accent-success text-white active:bg-accent-success/90 shadow-glow-purple'
            }`}
            whileHover={{ scale: isGenerating || !settings.checkpoint || !prompt.trim() ? 1 : 1.02 }}
            whileTap={{ scale: isGenerating || !settings.checkpoint || !prompt.trim() ? 1 : 0.98 }}
          >
            {isGenerating ? (
              <div className="flex items-center justify-center gap-3">
                <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin" />
                <span className="text-base md:text-lg">Generating... {generationProgress}%</span>
              </div>
            ) : (
        ...
        {/* Right Panel - Preview */}
        <div className="bg-bg-secondary rounded-lg border border-border-primary p-4 md:p-6 flex flex-col min-h-[400px] lg:min-h-[600px]">
          {/* Preview Header */}
          ...
          {/* Preview Content */}
          <div className="flex-1 bg-bg-tertiary rounded-lg border border-border-primary overflow-auto">
            {generatedImages.length > 0 ? (
                ...
                {isGenerating ? (
                  ...
                  <div>
                    ...
                    <p className="text-text-secondary text-base">
                      No preview yet. Generate some images to see them here!
                    </p>
                  </div>
                )}
              </div>
...
// Preview Lightbox Component with Keyboard Navigation
